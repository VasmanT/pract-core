...
CONTAINER_ID="${1:-7b5}"
PROJECT_ROOT="$(cd .. && pwd)"
JAR_PATH="${PROJECT_ROOT}/target/dbmicro-1.0-SNAPSHOT.jar"
SERVICE_PORT="8091"  # Порт вашего Spring приложения
...
main() {
    ...
    if docker cp "$JAR_PATH" "${CONTAINER_ID}:/app.jar"; then
#    if docker cp "$JAR_PATH" "${CONTAINER_ID}:/app/app.jar"; then
    ...
    print_info "5. Запускаем приложение"
    if docker exec -d "$CONTAINER_ID" sh -c "pkill -f 'java.*jar' 2>/dev/null || true; java -jar /app/app.jar"; then
