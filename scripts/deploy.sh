...
# Упрощенная функция деплоя сервиса
deploy_service() {
    local service_name=$1
    local container_name=${CONTAINERS[$service_name]}
    ...
    # Убиваем старый процесс Java
    print_info "Остановка старого процесса Java..."
    docker exec "$container_name" pkill -f "java.*jar" || true
    sleep 2

    # Запускаем приложение (максимально простой способ)
    print_info "Запуск Java приложения..."

    # Используем простую команду без создания файла
    docker exec -d "$container_name" sh -c "cd /app && java -jar app.jar --spring.profiles.active=$PROFILE --server.port=$port > app.log 2>&1"

    print_info "Ожидание запуска процесса..."
    sleep 5

    # Проверка запуска
    print_info "Проверка статуса..."
    if docker exec "$container_name" pgrep -f "java.*jar" &> /dev/null; then
        print_success "Процесс Java запущен в контейнере $container_name"
    else
        print_warning "Процесс Java не найден, проверьте логи"
        docker exec "$container_name" cat /app/app.log 2>/dev/null || echo "Лог файл не найден"
    fi
